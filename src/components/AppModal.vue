<template>
  <transition name="fade">
    <div
      class="flex items-stretch absolute w-full h-full z-50 left-0 top-0"
      v-if="isOpen"
    >
      <div
        @click="onToggle"
      ...
      ></div>
      <div
        class="self-center left-0 top-0 w-full max-w-lg p-4 relative mx-auto my-auto rounded-xl shadow-lg bg-gray-900"
      >
        <div class="text-center flex-auto justify-center leading-6">
          ...
          <p class="text-md text-gray-500 py-2 px-2">
            <slot name="body"
              >Do you really want to exit without saving your work?</slot
            >
          </p>
        </div>
        <div
          class="flex flex-row justify-end gap-2 pt-4 mt-4 border-t-2 border-gray-800"
        >
          <slot name="footer">
            <button
